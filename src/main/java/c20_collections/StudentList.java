...
//
//        // "박수빈" 이름을 가진 객체의 학번을 콘솔에 출력하시오 -> 검색과 관련있습니다. StringList.java 확인해서 만듦
...
//        for(int i =0; i< students.size(); i++) {
//            if(students.get(i).getId() == searchInt) {      // int나 String이냐에 따라 조건식 방식이 달라진 부분에 주목하세여
...
//        int searchInt = 20240002;
//        for(int i = 0; i< students.size(); i++) {
//            if(students.get(i).getId() == searchInt) {
//                System.out.println(searchInt + " 학번을 지닌 학생의 이름 : " + students.get(i).getName());
//            }
//        }
        for(Student student : students) {
            if(student.getId() == 20240002) {
                if(student.getName().equals("강준석")) {
                    student.setName("박승주");
                    System.out.println(student.getId() + " 학번의 학생의 이름이 " + student.getName() + "(으)로 바뀌었습니다.");
                    break;
                }
            }
        }
