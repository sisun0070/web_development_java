...
public class Main {
    public static void main(String[] args) {
        ...
        centralControl1.addDevice(tv1);

        // 현재 코드를 작성한 바에 따르면 객체를 centralControl에 추가했을 때만 콘솔창에 어떤 클래스의 객체가 추가되었는지
        // 확인하는 것만 가능한 상태입니다.

        // centralControl.displayInfo()를 실행했을 때 콘솔창에 현재 배열이 들어가있는 요소들을 출력할 수 있도록 코드를 작성하시오.
//        centralControl1.displayInfo();

//        centralControl1.displayInfo3();
        // 이상의 코드에서 생길 수 있는 문제점
        // displayInfo3()의 경우에는 배열 전체를 Arrays.toString()을 이용하여 배열 자체를 출력할 수 있게끔 작성함.
        // 예를 들어 배열 내무의 element가 기본 자료형이라면 그 값이 그대로 String으로 바뀌어서 쓰이게 되겠지만,
        // 현재와 같은 경우에는 'Power 클래스의 서브 클래스의 객체'가 element이기 때문에 참조 자료형이라 주소값이
