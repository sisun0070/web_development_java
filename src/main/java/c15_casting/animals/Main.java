        ...
        5. 캐스팅의 장단점
            ...
            단점 :
                다운 캐스팅의 위험성 : 잘못 다운 캐스팅을 하게 됩니다.
                 ...
                 복잡성 증가 : 코드 가독성이 떨어질 수 있음 -> 이렇게 바꿨다고 저렇게 바꿨다가 하기 때문인데
                    특히 업캐스팅의 경우 암시적으로 일어나기 때문에 어디서 바뀌었는지 확인은 못하고 있다가
                    갑자기 다운캐스팅이 튀어나오는 경우에 초심자들이 코드를 읽는게 힘겨울 수 있습니다.

        instanceof 연산자 : Java에서 객체의 실제 타입을 검샇는 데 사용(어떤 클래스의 객체인지를 확인하는 용도)
...
public class Main {
    public static void main(String[] args) {
        // Dog 객체 생성
        Dog dog1 = new Dog();   // -> 기본 생성자 ( 자동으로 만들어짐)
...
//        animal1.fetch();      // 타입의 Animal로 선언돼있기 때문에
...
//        animal2.fetch();      // animal1 때와 동일하게 호출 불가능함.
        ...
        Dog dog2 = (Dog) animal2;       // 다운캐스팅의 경우 명시적으로 쓰여야하기 때문에
                                        ...
                                        // 이제는 클래스와 객체 단위에서 생각할 필요가 있습니ㅏㄷ.
...
//        dog3.fetch();
        /*
            ...
            * Try:
            ...
            그 이유는 :
                애초에 Animal로 만든 것을 아무런 변형 벗이 강제적으로 다운캐스팅을 시도했기 때문입니다.

            이상과 같은 컴파일링 오류를 막기 위한 방식으로 코드를 아래와 같이 작성할 수 있습니ㅏㄷ.
        ...
        if (animal4 instanceof Dog){
            Dog dog4 = (Dog) animal4;       // 위의 조건문이 true일 때만 현재 코드라인이 실행되기 때문에
                                            // 컴파일링 오류가 발생하지 않음
            dog4.makeSound();
            dog4.fetch();
        }
