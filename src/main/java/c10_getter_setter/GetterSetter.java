    ...
    1. Setter / Getter의 필요성
        ...
        class PersonMain {
            public static void main(String[] args) {
                ...
                person.name = "안근수";
                person.age = -10;  // 의도적으로 이상한 나이를 설정함
            }
        }
        해당 코드를 확인했을 때 나이에 정상적이지 않은 값이 대입돼있습니다.
        이처럼 외부에서 직접 접근이 가능했을 때 이상한 값이 들어가더라도 통제할 수 있는 방법이 없는 문제가 있습니다.
        -> 이를 해결하기 위한 방법 : Setter / Getter 개념
        ...
        2. Setter/ Getter란?
            ...
            Getter
                ...
                여태까지 수업한 전체 패키지 및 파일 확인에서 내용 확인 화려면
                -> ctrl + shift + f

                해결책 : 캡슐화(encapsulation)
                클래스 내부의 데이터를 외부에-서 바로 접근하지 못하게 하고, 대신;
                "메서드"를 통해서 간접적으로 접근하도록 만드는 것.

                "이를 통해 데이터의 유효성을 검증하거나 특정한 로직을 추가"할 수 있습니다.

         3. Sstter와 Getter의 장점
            1) 데이터 보호
         ...
         4. 그래서 Setter / Getter는 언제나 필수적인가?
            모든 경우에 Setter / Getter가 필수적인 것은 '아닙니다,'
