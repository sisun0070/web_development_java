    ...
    Lombok
        ...
        장점 :
            코드의 가독성을 높일 수 있다. -> 주석을 통한 애너테이션을 붙여 코드 생성을 제어 가능
                ...
                유무 등을 고려할 필요가 없어 @ 파트만 보더라도 대략적인 코드 흐름을 알 수 있음.

        1. @Data
            - @Getter, @Setter, @RequiredArgsConstructor, @ToString, @EqualAndHashCode
                에너테이션들을 포함한 종합 패키지에 해당
        ...
        4. @Getter / @Setter
            ...
            - 클래스 레벨이 사용하면 모든 필드에 대해, 필드 레벨에 사용하면 특정 필드에 대해서면
                getter와 setter가 생성됨 -> 저희는 예시로 보여드렸을 때 클래스 레벨에만 적용했었습니다.
            - 단 @Setter 기준으로 fianl 필드에는 Setter가 생성되지 않습니다.
